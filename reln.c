...
void clear_page(Reln r, PageID p, int mode) {
	// clear the page specified by the pageId and the mode
	// if mode is 0, means data file, if 1 means ov file
	Page pg;
	if (mode == 0) {
		pg = getPage(dataFile(r), p);
	} else {
		pg = getPage(ovflowFile(r), p);
	}
	pg->free = 0;
	pg->ntuples = 0;
	int dataSize = PAGESIZE - (2 * sizeof(Offset) + sizeof(Count));
	memset(pg->data, 0, dataSize);
	if (mode == 0) {
		putPage(dataFile(r), p, pg);
	} else {
		putPage(ovflowFile(r), p, pg);
	}
}

Bool raw_addToPage(Reln r, PageID p, Tuple t) {
	// this version just write the tuple to the page
	// the parameter should be the main page
	// but this function can potentially write the tuple 
	...
	if (addToPage(pg,t) == OK) {
		...
		if (pageOvflow(pg) == NO_PAGE) {
			...
			if (addToPage(newpg,t) != OK) return FALSE;
...
PageID addToRelation(Reln r, Tuple t)
{
	...
	// char buf[MAXBITS+1];
	h = tupleHash(r,t);
	int c = 1024 / (10 * nattrs(r));
	// int c = 60;
	// I think the cnt can be represented by the total number of tuples in the relation
	PageID result = NO_PAGE;
	// result = 1;
	if (r->depth == 0)
		p = 0;//why 1
	else {
    ...
    if (r->ntups % c == 0) {
    	...
    	r->npages++;
    	// get the sp page
    	PageID sp_pageid = splitp(r);
    	Page sp_page = getPage(dataFile(r), splitp(r));
    	// for all tuples in the sp page and its ov page
    	// check the hash value, and send to corresponding pages

    	// first, sp_page
    	// clear the page
    	clear_page(r, sp_pageid, 0);
    	// for all tuples in sp_page
    	Count num_tuples = pageNTuples(sp_page);
    	char *ptc = pageData(sp_page);
    	for (int tuple_index = 0; tuple_index < num_tuples; tuple_index++) {
    		Tuple current_tuple = ptc;
    		// calculate the hash value of the tuple
    		Bits current_hash = tupleHash_silent(r,current_tuple);
    		Bits current_p = getLower(current_hash, r->depth+1);
    		// consider the destination of this tuple
    		if (current_p == new_pageid) {
    			// add to new page
    			raw_addToPage(r, new_pageid, current_tuple);
    		} else {
    			// add to sp page
    			raw_addToPage(r, sp_pageid, current_tuple);
    		}

    		ptc += strlen(ptc) + 1;
    	}

    	// then, the overflow pages, if exist
    	Page prev_page = sp_page;
    	PageID ovp;
    	while (pageOvflow(prev_page) != NO_PAGE) {
    		ovp = pageOvflow(prev_page);
    		Page cur_ov_page = getPage(r->ovflow, ovp);
    		clear_page(r, ovp, 1);
    		Count num_tuples = pageNTuples(cur_ov_page);
    		char *ptc = pageData(cur_ov_page);
    		for (int tuple_index = 0; tuple_index < num_tuples; tuple_index++) {
	    		Tuple current_tuple = ptc;
	    		// calculate the hash value of the tuple
	    		Bits current_hash = tupleHash_silent(r,current_tuple);
	    	...
	    	Page tmp = prev_page;
	    	prev_page = cur_ov_page;
	    	free(tmp);
    	}
    	r->sp++;
    	if (r->sp == self_pow(r->depth)) {
    		r->depth++;
    		r->sp = 0;
    	}
    }
