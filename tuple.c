...
Bits tupleHash_silent(Reln r, Tuple t)
{
	char buf[MAXBITS+1];
	...
	assert(vals != NULL);
	tupleVals(t, vals);
	// calculate hash value of all attributes
	Bits *hash_ptr = malloc(nvals * sizeof(Bits));
	for (int i = 0; i < nvals; i++) {
		// note: each Bits value is in base 10
		*(hash_ptr+i) = hash_any((unsigned char*)vals[i], strlen(vals[i]));
	}
	// get the cv, which is an array of ChVecItem
	ChVecItem* choice_vector = chvec(r);
	Bits hash = 0;
	for (int i = 0; i < MAXCHVEC; i++) {
		int attribute_index = (choice_vector+i)->att;  // type cast from Byte to int
		int bit_index = (choice_vector+i)->bit;  // type cast from Byte to int
